...
\begin{definition}[Expected value]
	...
	Let $X$ be a real-valued random variable defined on a probability space $(\Omega, \mathcal{F},\mathbb{P})$, then the expected value of $X$, denoted by $\mathbb{E}[X]$, is defined by the Lebesgue integral~\citep{taboga2021}
	\begin{equation}
...
\end{theorem}
\begin{proof}
	Let $\mathbf{1}_{\{X\ge a\}}$ denote the indicator of the event $\{X\ge a\}$. Since $X$ is non-negative and $a>0$
	\begin{equation}
		a\mathbf{1}_{\{X\ge a\}} \le X.
	\end{equation}
	Taking expectations and using \thref{theorem:exp_linear}
	\begin{equation}
		a\,\mathbb{E}\big[\mathbf{1}_{\{X\ge a\}}\big] \le \mathbb{E}[X].
	\end{equation}
	But $\mathbb{E}[\mathbf{1}_{\{X\ge a\}}]=\mathbb{P}(X\ge a)$, so
	\begin{equation}
...
\begin{definition}[Probability Density Function]
	...
	\begin{equation}
		...
		\label{eq:cont}
	\end{equation}
	 where $\int_{-\infty}^\infty f(X=t) dt = 1$ and $f(X=t)=0$ for any individual point $t \in \mathbb{R}$.
\end{definition}
...
\begin{theorem}[Expected value of continuous random variable]
	...
	\begin{equation}
		\mathbb{E}[X]=\int_{\Omega_X}xf(X =x)dx.
	\end{equation}
\end{theorem}
