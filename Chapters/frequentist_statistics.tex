...
\begin{example}
	...
	\index{Example: Bernoulli parameter estimation}
	Consider a dataset $D_y = \{y_i\}_{i=1}^n$ consisting of independent and identically distributed (IID)\index{IID} random variables
	\begin{equation}
		Y_i \sim \mathrm{Ber}(w), \, w \in [0,1]
	\end{equation}
	and the goal of estimating the unknown parameter $w$. 
	\begin{itemize}
...
\begin{example}
	...
	\begin{equation} 
		\begin{split} 
			\argmax_wl(w) & = \argmax_wp(D_y|w)\\ &= \argmax_w\ln \bigg(\prod_{i=1}^nw^{y_i}(1-w)^{1-y_i}\bigg)\\ &=\argmax_w \bigg[\ln w\sum_{i=1}^ny_i + \ln(1-w)\sum_{i=1}^n(1-y_i)\bigg] 
		\end{split} 
	\end{equation} 
	Now 
	\begin{equation} 
	...
	\end{equation} 
	Requiring the derivative to vanish means the maximum likelihood estimate\index{Maximum likelihood} of $w$ is given by 
	\begin{equation} 
...
\begin{example}
	...
	Consider a dataset $D_y = \{y_i\}_{i=1}^n$ consisting of independent and identically distributed (IID)\index{IID} random variables
	\begin{equation}
		Y_i \sim \mathrm{Exp}(w), \, w>0.
	\end{equation}
	The likelihood function is
	\begin{equation}
		p(D_y \mid w) = \prod_{i=1}^n w e^{-w y_i}.
	\end{equation}
	The log-likelihood is
	\begin{equation}
		l(w) = n \ln w - w \sum_{i=1}^n y_i.
	\end{equation}
	Differentiating and setting to zero gives the maximum likelihood estimate\index{Maximum likelihood}:
	\begin{equation}
		\hat{w}_{\mathrm{MLE}}(D_y) = \frac{n}{\sum_{i=1}^n y_i} = \frac{1}{\bar{y}},
	\end{equation}
