...
\begin{example}
	With reference to \exref{ex:rain}, the possible states of Nature are $s_1 = "\text{rain}"$ and $s_2 = "\text{no rain}"$, whereas each observed outcome $s^{(i)}$ in the dataset 
	\begin{equation}
...
\begin{example}
	...
	\end{equation}
	The Area Under the ROC\index{AUROC}\index{Area Under the ROC} Curve (AUROC) is a threshold-independent measure. Let $X_{(s_1)}$ and $X_{(s_2)}$ denote independent draws from the class-conditional distributions $p(x | S = s_1)$ and $p(x | S = s_2)$, respectively. Then
	\begin{equation}
...
\begin{example}
	...
	\begin{enumerate}
		...
		The conditional expected cost if the robot does not pick the reject option, meaning $U(\tilde{D})\in \mathbb{U}\setminus\text{reject}$
		\begin{equation}
			\begin{split}
		...
		\begin{equation}
			...
			\label{eq:cost2a}
		\end{equation}
		Equation \eqref{eq:cost1a} show picking $\arg\max_{U(\tilde{D})\in \mathbb{U}\setminus \text{reject}} p(S=U(\tilde{D})|\tilde{D},I)$ is the best option among classes $U(\tilde{D})\neq \text{reject}$. To be the best option overall, it also needs to have lower cost than the reject option. Using equations \eqref{eq:cost1} and \eqref{eq:cost2a} yields
		\begin{equation}
