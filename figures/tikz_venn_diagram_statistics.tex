...
\begin{document}
	\begin{tikzpicture}[
		...
		\node[right = 2.5cm of F] (unit) {$[0,1]$};
		\node[right = 2.5cm of X] (P1) {$P_w$};
		
		\node[set, above = of P1, label={above:$\mathcal{P}$}] (P) {$P_w$};		
		\node[set, above = of Y, label={above:$\mathbb{W}$}] (G) {$w$};
		\node[set, left = of G, label={above:$\Gamma$}] (gamma) {$\gamma$};
		
		\node[right = 3cm of unit] (R) {$\mathbb{R}_{\geq 0}$};
		...
		\draw[arrow] (P1) -- node[left = 2mm] {$p: \mathbb{X}\times \mathbb{Y}\times \mathbb{W} \mapsto [0,1]$} (unit);
		\draw[arrow] (P1) -- node[right] {$f: \mathbb{X}\times \mathbb{Y}\times \mathbb{W} \mapsto [0,\infty)$} (R);
